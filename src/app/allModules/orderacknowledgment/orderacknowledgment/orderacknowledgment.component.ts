...
export class OrderacknowledgmentComponent implements OnInit {
  ...
  // GetAcknowledgmentDetailvalues(): void {
  //   this.AcknowledgementDetails.POOrderScheduleLine = [];
  //   const AcknowledgementFormArr = this.OrderAcknowlegmentGroup.get('POOrderScheduleLine') as FormArray;
  //   AcknowledgementFormArr.controls.forEach((x, i) => {
  //     const item: POOrderScheduleLine = new POOrderScheduleLine();
  //     item.PO = x.get('PO').value;
  //     item.Item = x.get('Item').value;
  //     item.TransID = x.get('TransID').value;
  //     item.ScheduleLine = x.get('ScheduleLine').value;
  //     item.Status = x.get('Status').value;
  //     item.OrderQuantity = x.get('OrderQuantity').value;
  //     item.AcceptedQuantity = x.get('AcceptedQuantity').value;
  //     item.DeliveryDate = x.get('DeliveryDate').value;
  //     item.AcceptedDate = x.get('AcceptedDate').value;
  //     item.Created_On = x.get('Created_On').value;
  //     // item.Created_By = x.get('Created_By').value;
  //     item.Approved_On = x.get('Approved_On').value;
  //     item.Approved_By = x.get('Approved_By').value;
  //     item.IsActive = x.get('IsActive').value;
  //     this.AcknowledgementDetails.POOrderScheduleLine.push(item);
  //   })
  //   console.log(AcknowledgementFormArr);
  // }
  getRowValues(data): any {
    this.emptyList();
    data.forEach(element => {
      this.AcknowledgementDetails.POOrderScheduleLine.push(element.value);
      // console.log(this.AcknowledgementDetails.POOrderScheduleLine);
    });
    //this.AcknowledgementDetails.POOrderScheduleLine = data;
    console.log(this.AcknowledgementDetails.POOrderScheduleLine);
  }
  emptyList() {
    this.AcknowledgementDetails.POOrderScheduleLine = [];
  ...
  submitASNDetails(status: string): void {
    ...
    //this.AcknowledgementDetails.POOrderScheduleLine = this.selection.selected;
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.dashboardService.CreateOrderAcknowledgement(this.AcknowledgementDetails).subscribe((data) => {
          ...
          this.dashboardService.AddOrderAcknowledgementAttachment(aux, this.fileToUploadList).subscribe(
            (dat) => {
              ...
              this.notificationSnackBarComponent.openSnackBar('Acknowledgement details updated successfully', SnackBarStatus.success);
